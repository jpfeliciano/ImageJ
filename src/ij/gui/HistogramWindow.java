...
public class HistogramWindow extends ImageWindow implements Measurements, ActionListener,
	...
	public void showHistogram(ImagePlus srcImp, ImageStatistics stats) {
		if (srcImp.isRGB() && rgbMode < HistogramConstants.INTENSITY1)
			rgbMode = HistogramConstants.INTENSITY1;
	...
	void drawAlignedColorBar(ImagePlus imp, double xMin, double xMax, ImageProcessor ip, int x, int y, int width,
		...
		if (rgbMode >= HistogramConstants.INTENSITY1) {
			ipRamp = new FloatProcessor(width, height);
			if (rgbMode == HistogramConstants.RED)
				ipRamp.setColorModel(LUT.createLutFromColor(Color.red));
			else if (rgbMode == HistogramConstants.GREEN)
				ipRamp.setColorModel(LUT.createLutFromColor(Color.green));
			else if (rgbMode == HistogramConstants.BLUE)
	...
	private void changeChannel() {
		...
		if (imp == null || !imp.isRGB())
			...
			rgbMode++;
			if (rgbMode > HistogramConstants.BLUE)
				rgbMode = HistogramConstants.INTENSITY1;
			ColorProcessor cp = (ColorProcessor) imp.getProcessor();
			boolean weighted = cp.weightedHistogram();
			if (rgbMode == HistogramConstants.INTENSITY2) {
